...
const loadHashBasedModeData = async () => {
  ...
  // 1. 获取实时文件映射
  const realTimeMap = await getAllFilesDateMap();
  ...
  if (!data) {
    ...
    if (latestRemoteData) {
      ...
      // 用实时数据填充本地数据
      for (const [date, hashSet] of Object.entries(realTimeMap)) {
        tempLocalData.dailyEdits[date] = Array.from(hashSet);
      }
      
      // 合并本地实时数据和远程缓存数据
      data = await mergeDataSafely(tempLocalData, latestRemoteData);
    } else {
      // 如果读取失败,直接用实时数据
      data.dailyEdits = {};
      for (const [date, hashSet] of Object.entries(realTimeMap)) {
...
const loadHeatmapData = async () => {
  ...
  // 📄 后台任务: 无论什么模式,都在后台维护 data.json (哈希记录模式)
  (async () => {
    try {
      const realTimeMap = await getAllFilesDateMap();
      let data = await readDataFile();
