/**
 * ============================================================================
 * HeatmapRadarCombined.js — 热力图 + 雷达图组合视图(混合模式优化版)
 * ============================================================================
 * 使用说明:
 * ```dataviewjs
...
const readDataFile = async () => {
  try {
    ...
    if (!data.version) data.version = 5; // 混合模式版本号
...
const getRealTimeTodayCount = () => {
  ...
  return todayFiles.size;
};

/**
 * 🆕 混合模式: 历史用缓存 + 今天实时统计
 */
const loadHybridModeData = async () => {
  const today = getTodayString();
  ...
  // 2. 实时统计今天的数据(确保准确)
  const realTimeTodayCount = getRealTimeTodayCount();
  
  if (CONFIG.debug) {
    const cachedCount = data.dailyEdits[today] || 0;
    console.log(`📊 今日数据对比: 缓存=${cachedCount}, 实时=${realTimeTodayCount}`);
  }
  
  // 3. 用实时数据覆盖今天的缓存
  data.dailyEdits[today] = realTimeTodayCount;
  
  // 4. 异步更新缓存(不阻塞显示)
  (async () => {
    try {
      // 再次读取文件,检查是否被其他设备更新(解决同步冲突)
      const latestData = await readDataFile();
      
      if (latestData) {
        // 合并策略: 对所有日期取最大值
        for (const [date, count] of Object.entries(latestData.dailyEdits)) {
          if (date === today) {
            // 今天的数据用实时统计的
            data.dailyEdits[date] = realTimeTodayCount;
          } else {
            // 历史数据取最大值
            data.dailyEdits[date] = Math.max(
              data.dailyEdits[date] || 0,
              count
            );
          }
        ...
        if (CONFIG.debug) {
          console.log('🔄 已合并其他设备的数据');
        }
      }
      
      await writeDataFile(data);
      if (CONFIG.debug) console.log('💾 缓存已更新');
    } catch (e) {
      console.warn('⚠️ 后台更新缓存失败:', e);
    }
  })();
...
const loadHeatmapData = async () => {
  ...
  // 🔄 后台任务: 无论什么模式,都在后台维护 data.json (不阻塞显示)
  (async () => {
    try {
      let data = await readDataFile();
  ...
  if (!type || type === 'default') {
    if (CONFIG.debug) console.log('🔄 使用混合模式 (历史缓存 + 今日实时)');
    return await loadHybridModeData();
  } else if (type === 'cday') {
    ...
    console.warn(`⚠️ 未知的热力图类型: ${type},使用混合模式`);
    return await loadHybridModeData();
  }
};
