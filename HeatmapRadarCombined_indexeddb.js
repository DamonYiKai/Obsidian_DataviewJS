/**
 * ============================================================================
 * HeatmapRadarCombined.js — 热力图 + 雷达图组合视图(IndexedDB优化版)
 * ============================================================================
 * 使用说明:
 * ```dataviewjs
 ...
 * 
 * 热力图模式说明:
 * - default (默认): 使用 IndexedDB 存储历史数据,自动增量更新
 * - cday: 实时统计笔记创建日期
 * - mday: 实时统计笔记修改日期
 * 
 * IndexedDB 配置:
 * - dbName: 数据库名称,默认 "ObsidianHeatmapDB"
 * - storeName: 存储名称,默认 "dailyEdits"
...
const CONFIG = {
  ...
  /* ===== IndexedDB 配置 ===== */
  dbName: "ObsidianHeatmapDB",
  storeName: "dailyEdits",
  dbVersion: 1,
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(CONFIG.storeName)) {
        db.createObjectStore(CONFIG.storeName, { keyPath: 'date' });
      }
    };
  });
};
...
/**
 * 从 IndexedDB 读取所有数据
 * 返回格式: { dailyEdits: { "2025-01-01": 5, ... }, lastUpdate: "2025-01-01 12:00:00", version: 4 }
 */
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        // 转换数据格式: [{date: "2025-01-01", count: 5}, ...] -> {"2025-01-01": 5, ...}
        records.forEach(record => {
          if (record.date && typeof record.count === 'number') {
            dailyEdits[record.date] = record.count;
          }
          // 获取最新的更新时间
          if (record.lastUpdate) {
            if (!lastUpdate || record.lastUpdate > lastUpdate) {
        ...
        if (CONFIG.debug) {
          console.log('从 IndexedDB 读取数据:', {
            总天数: Object.keys(dailyEdits).length,
            最后更新: lastUpdate
          });
        }
        ...
        resolve({
          dailyEdits,
          lastUpdate: lastUpdate || getCurrentTimestamp(),
          version: 4
        });
      };
    });
    ...
    return { dailyEdits: {}, lastUpdate: null, version: 4 };
  }
};

/**
 * 写入数据到 IndexedDB
 * 输入格式: { dailyEdits: { "2025-01-01": 5, ... }, lastUpdate: "...", version: 4 }
 */
const writeToIndexedDB = async (data) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 转换数据格式: {"2025-01-01": 5, ...} -> [{date: "2025-01-01", count: 5, lastUpdate: "..."}, ...]
      const records = Object.entries(data.dailyEdits || {}).map(([date, count]) => ({
        date,
        count,
      ...
      records.forEach(record => {
        ...
        req.onsuccess = () => {
          ...
          if (completed === records.length) {
            db.close();
            if (CONFIG.debug) {
              console.log('数据已保存到 IndexedDB:', timestamp);
            }
            resolve(true);
          }
        };
...
/**
 * 加载默认模式数据 (使用 IndexedDB)
 */
const loadDefaultModeData = async () => {
  let data = await readFromIndexedDB();
  
  // 如果 IndexedDB 为空,执行全量扫描
  if (!data.dailyEdits || Object.keys(data.dailyEdits).length === 0) {
    if (CONFIG.debug) console.log('IndexedDB 为空,执行全量扫描');
...
const loadHeatmapData = async () => {
  ...
  // 📄 后台任务:无论什么模式,都在后台更新 IndexedDB(不阻塞主流程)
  (async () => {
    try {
      let data = await readFromIndexedDB();
      if (!data.dailyEdits || Object.keys(data.dailyEdits).length === 0) {
        if (CONFIG.debug) console.log('[后台] IndexedDB 为空,执行全量扫描');
        data = await fullScanAllFiles();
        await writeToIndexedDB(data);
      } else {
        await updateTodayData(data);
      }
      if (CONFIG.debug) console.log('[后台] IndexedDB 更新完成');
    } catch (e) {
      console.warn('[后台] IndexedDB 更新失败:', e);
    }
  })();
  
  // 📊 前台显示:根据用户选择的模式返回数据
  if (!type || type === 'default') {
    if (CONFIG.debug) console.log('使用 default 模式 (IndexedDB)');
